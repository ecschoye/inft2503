...
public:
  ...
  void updatePosition(float h, float v ) {
    ...
    restart();
    transform.setOrigin(btVector3(h, v, 0.0));
    falling_sphere.body.setCenterOfMassTransform(transform);
  }

  void addGravity() {
    falling_sphere.body.setGravity(btVector3(0, -9.81, 0));
    falling_sphere.body.setLinearVelocity(btVector3(0, 1, 0));
    std::cout << "addGravity" << std::endl;
  }
    ...
    while (running) {
      ...
      if (ImGui::Button("Drop ball")) {
        ...
        if (!dropped) {
          world.addGravity();
          dropped = true;
                std :: cout << "Dropped ball" << std :: endl;
        }
      }
      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, 0.0, 3.0)) {
        ...
        dropped = false;
        world.restart();
        world.updatePosition(horizontal_position, vertical_position);
        std::cout << "Horizontal ball position: " << horizontal_position << std::endl;
      }
      if (ImGui::VSliderFloat("Vertical ball position", {20, 50}, &vertical_position, 0.0, 4.0)) {
        // Implementation needed
        dropped = false;
        world.restart();
        world.updatePosition(horizontal_position, vertical_position);
        std::cout << "Vertical ball position: " << vertical_position << std::endl;
      }
